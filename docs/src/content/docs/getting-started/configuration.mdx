...
Root-absolute hrefs get the site `base` prefixed, so they keep working when a
pack is served from a sub-path. An entry can also point at another site —
absolute (`https://…`, `mailto:…`) and protocol-relative (`//host/path`) hrefs
are rendered verbatim and open in a new tab, and they never take the active-tab
state, since no page of this site can match them:
...
The tabs render in the header, the mobile drawer, and the footer's
